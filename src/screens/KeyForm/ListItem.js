...
class ListItem extends Component {
  ...
  render() {
    const { onDelete, carKey } = this.props;
    ...
    return (
      <View style={styles.item}>
        ...
        <View style={styles.info}>
          <Text style={styles.name}>{carKey.ownerName}</Text>
          <View style={styles.exp}>
            <Text style={styles.years}>{carKey.rfid}</Text>
            <Text style={styles.subtitle}>Tarjeta</Text>
          </View>
        </View>
        <TouchableOpacity onPress={() => onDelete(carKey)} activeOpacity={0.5}>
