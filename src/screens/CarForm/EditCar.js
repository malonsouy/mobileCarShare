...
import {removeKey, saveScreenData, navigate, goBack} from "../../actions";
...
class EditCar extends Component {
      ...
      <KeyboardAwareScrollView ref="scroll" style={styles.scroll}>
        ...
        <View style={styles.screen}>
          ...
          <View style={styles.form}>
            ...
            {keys.map(key => {
              return (
                <ListItem key={key.id} carKey={key} onRemove={this._onRemoveKey.bind(this)} />
              );
            })}
          </View>
          <View style={styles.form}>
  ...
  _onRemoveKey(keyRemoved) {
    const {removeKey} = this.props;
    const {keys} = this.state;
    removeKey(keyRemoved.id);
    let newKeys = keys.filter(key => { return key.id !== keyRemoved.id });
    this.setState({keys: newKeys});

  }

  _addKey() {
    const { navigation: { car, navigate } } = this.props;
    navigate("AddKey", {car} );
  }
...
const mapDispatchToProps = dispatch => {
  return {
    removeKey: data => dispatch(removeKey(data)),
